...
//        final int size = MPI.COMM_WORLD.Size();
        ...
        if (rank == MASTER) {
            ...
            for (int i = 1; i < workerCount + 1; i++) {
                ...
                MPI.COMM_WORLD.Send(new int[]{rowsPerProcess}, 0, 1, MPI.INT, i, Utilitary.FIRST_ROWS_NUM_TAG);
                MPI.COMM_WORLD.Send(matrixA, offset, rowsPerProcess, MPI.OBJECT, i, Utilitary.FIRST_MATRIX_TAG);
                MPI.COMM_WORLD.Send(matrixB, 0, MATRIXSIZE, MPI.OBJECT, i, Utilitary.SECOND_MATRIX_TAG);

                extra--;
                offset += rowsPerProcess;
            }
            for (int i = 1; i < workerCount + 1; i++) {
                MPI.COMM_WORLD.Recv(matrixC, offsets[i - 1], rows[i - 1], MPI.OBJECT, i, Utilitary.RES_TAG);
            }
            ...
            MPI.COMM_WORLD.Recv(rowsPerProcess, 0, 1, MPI.INT, MASTER, Utilitary.FIRST_ROWS_NUM_TAG);
            ...
            MPI.COMM_WORLD.Recv(rowsA, 0, rowsPerProcess[0], MPI.OBJECT, MASTER, Utilitary.FIRST_MATRIX_TAG);
            MPI.COMM_WORLD.Recv(rowsB, 0, MATRIXSIZE, MPI.OBJECT, MASTER, Utilitary.SECOND_MATRIX_TAG);
            ...
            MPI.COMM_WORLD.Send(result, 0, result.length, MPI.OBJECT, MASTER, Utilitary.RES_TAG);
        }
